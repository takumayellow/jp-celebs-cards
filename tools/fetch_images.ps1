...
Write-Log "Done."
function Is-FreeLicense([string]$lic){
  if([string]::IsNullOrWhiteSpace($lic)){ return $false }
  return ($lic -match 'CC\s*-?\s*BY' -or $lic -match 'CC\s*-?\s*BY-?SA' -or
          $lic -match 'Public\s*Domain|PD' -or $lic -match 'GFDL')
}

function Get-WikiPageImageInfo([string]$title, [string]$lang='ja'){
  if([string]::IsNullOrWhiteSpace($title)){ return $null }
  try{
    # 1) ページの先頭画像の「ファイル名」を取る
    $u1 = "https://$lang.wikipedia.org/w/api.php?action=query&prop=pageimages&format=json&piprop=name|thumbnail|original&pithumbsize=1000&titles=$([Uri]::EscapeDataString($title))"
    $r1 = Invoke-RestMethod $u1 -TimeoutSec 20
    $page = $r1.query.pages.PSObject.Properties.Value | Select-Object -First 1
    $fname = $page.pageimage
    if(-not $fname){ return $null }

    # 2) そのファイルのライセンス/作者/クレジットとフルサイズURLを取る
    $fileTitle = "File:" + ($fname -replace ' ','_')
    $u2 = "https://$lang.wikipedia.org/w/api.php?action=query&titles=$([Uri]::EscapeDataString($fileTitle))&prop=imageinfo&iiprop=url|extmetadata&format=json&iiextmetadatafilter=Artist|Credit|LicenseShortName|LicenseUrl"
    $r2 = Invoke-RestMethod $u2 -TimeoutSec 20
    $p2 = $r2.query.pages.PSObject.Properties.Value | Select-Object -First 1
    if(-not $p2.imageinfo){ return $null }
    $ii = $p2.imageinfo[0]
    return [pscustomobject]@{
      ...
      Credit   = ($ii.extmetadata.Credit.value -replace '<[^>]+>','')
